...
function App() {
  ...
  async function cancelCurrentTask() {
    ...
    const response = await fetch(`/api/tasks/${current.id}/cancel`, { method: "POST" });
    if (!response.ok) return;
    const saved = await response.json();
    setDetail(saved);
    setRunning(false);
    setCurrent((item) => ({ ...item, status: "cancelled" }));
    setTasks((items) => items.map((item) => item.id === current.id ? { ...item, status: "cancelled" } : item));
    setMessages(saved.messages?.map((message) => ({ role: message.role, text: message.content })) || []);
  }
...
function RunPanel({ detail, running, current, onCancel }) {
